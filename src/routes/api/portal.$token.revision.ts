...
const revisionSchema = z.object({
  ...
  note: z.string().min(1, "Revision note is required").max(1000),
});

export const Route = createFileRoute("/api/portal/$token/revision")({
  server: {
    handlers: {
      POST: async ({ request, params }) => {
        try {
          ...
          // Fetch project to update status to in_progress / review pending
          ...
          if (!projectsSnap.empty) {
            ...
            await projectDoc.ref.update({
              ...
              status: "in_progress", // Revert status to work on revision
              updatedAt: FieldValue.serverTimestamp(),
            });
          }

          // Try to find the task and update its status to "in_progress" in Firestore
          ...
          try {
            ...
            console.error("Error updating task status to in_progress:", taskErr);
          }
          ...
          await adminDb
            ...
            .add({
              content: `⚠️ Revision requested on: "${milestoneName}"\nNotes: "${parsed.data.note}"`,
          ...
          console.error("[POST /api/portal/$token/revision]", err);
